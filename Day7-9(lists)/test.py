...
def get_state_abbrev(state_name, us_state_abbrev=us_state_abbrev):
    ...
    return us_state_abbrev.get(state_name) if us_state_abbrev.get(state_name) != None else NOT_FOUND
...
def get_longest_state(data):
    ...
    if type(data) is dict:
        data = list(data.keys())
    lenthes = [len(i) for i in data]
    id = lenthes.index(max(lenthes))
    return data[id]
...
def combine_state_names_and_abbreviations(us_state_abbrev=us_state_abbrev,
    ...
    """Get the first 10 state abbreviations ('AL', 'AK', 'AZ', ...) from
       ...
       alphabetical order"""
    abb = list(us_state_abbrev.values())[:10]
    st = sorted(states)[-10:]
    return abb + st

print(get_every_nth_state())
get_state_abbrev('Ohio')
